...
const WorkSchedulePage: React.FC = () => {
  ...
  const fetchTechnicians = async () => {
    ...
    merged.forEach((t, i) => { if (t.id!=='all') techColors[t.id] = colors[i % colors.length]; });
  ...
  const fetchSchedule = async (techId: string) => {
    if (techId === 'all') {
      ...
      const map: Modifiers = {};
      (data||[]).filter(d=>d.is_working).forEach(d=>{
      ...
      const { data } = await supabase.from('work_schedules').select('*').eq('technician_id', techId);
      setWorkDays((data||[]).filter(d=>d.is_working).map(d=>new Date(d.date)));
      setAllSchedules({});
    }
  };
  ...
  const vacationDays = requests
    ...
    .flatMap(r=>{
      const s=new Date(r.startDate), e=new Date(r.endDate), arr: Date[]=[];
  ...
  const saveWorkDays = async () => {
    ...
    const inserts = workDays.map(d=>({ technician_id:selectedTech, date:d.toISOString().slice(0,10), is_working:true }));
  ...
  const updateStatus = async (id:string, status:'approved'|'denied') => {
    const { error } = await supabase.from('vacation_requests').update({ status, approved_by:user?.id }).eq('id', id);
  ...
  const classes: ModifierClasses = { work:'bg-red-500 text-white', vacation:'bg-green-500 text-white' };
  Object.keys(allSchedules).forEach(id => classes[id]=techColors[id]+' text-white');
          ...
          <CardContent>
            <div className="relative">
              <Calendar
                ...
                onSelect={v=>{ if(isPlanner&&selectedTech!=='all') setWorkDays(v as Date[]); }}
                modifiers={modifiers}
                modifiersClassNames={classes}
                classNames={{ day_selected:'ring-2 ring-offset-2 ring-gray-500' }}
              />
              <div className="absolute top-2 right-2">
  <Button variant="ghost" size="sm" onClick={() => setFullscreen(true)}>Fullscreen</Button>
</div>
{fullscreen && (
  <div className="fixed inset-0 bg-white z-50 flex flex-col">
    <div className="flex justify-end p-4">
      <Button variant="ghost" onClick={() => setFullscreen(false)}>✕</Button>
    </div>
    <div className="flex-1 overflow-auto p-6">
      ...
      />
        ...
        {isPlanner&&selectedTech!=='all'&&(
          <Button onClick={saveWorkDays} className="mt-4">Opslaan Werkdagen</Button>
        )}
          ...
          <CardContent>
            <Tabs value={requestTab} onValueChange={v=>setRequestTab(v as RequestTab)}>
              ...
              <TabsContent value="approved">
                {requests.filter(r=>r.status==='approved').map(r=>(<div key={r.id}>{r.technicianName} {r.startDate} - {r.endDate}</div>))}
              </TabsContent>

              <TabsContent value="denied">
                {requests.filter(r=>r.status==='denied').map(r=>(<div key={r.id}>{r.technicianName} {r.startDate} - {r.endDate}</div>))}
              </TabsContent>
